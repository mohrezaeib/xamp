...
function vp_get_products_categories()
{
	...
	//$wp_cat = get_option( '_bunch_product_category_terms' );
	$result = array();
	foreach ($wp_cat as $cat)
	{
...
function vp_get_et_icons()
{
		$pattern = '/\.(flaticon-(?:\w+(?:-)?)+):before\s*{\s*content/';
		//$pattern = '/\.(fa-(?:\w+(?:-)?)+):before\s+{\s*content:\s*"(.+)";\s+}/';
		//$subject = file_get_contents(VP_DIR . '/public/css/vendor/font-awesome.min.css');
		$subject = file_get_contents(get_template_directory() . '/css/flaticon.css');
		preg_match_all($pattern, $subject, $matches, PREG_SET_ORDER);
		/*$icons = array();
		foreach($matches as $match)
		{
...
VP_Security::instance()->whitelist_function('bunch_get_sidebars_2');
function bunch_get_sidebars_2()
{
	global $wp_registered_sidebars;
	$sidebars = !($wp_registered_sidebars) ? get_option('wp_registered_sidebars') : $wp_registered_sidebars;
	$data =  array(array('value'=> '', 'label'=> __('No Sidebar', BUNCH_NAME)));
	foreach( (array)$sidebars as $sidebar)
	{
		$data[] = array('value'=> bunch_set($sidebar, 'id'), 'label'=>bunch_set($sidebar, 'name')) ;
	}
	return $data;
}
/*------------------------------------------------*/
...
VP_Security::instance()->whitelist_function('vp_dep_style4');
VP_Security::instance()->whitelist_function('vp_dep_style10');
VP_Security::instance()->whitelist_function('vp_dep_style11');
function vp_dep_style2($value)
{
	if( $value == 'header_v2' ) return true;
	
	return false;
}
function vp_dep_style1($value)
{
	if( $value == 'header_v1' ) return true;
	
	return false;
}
function vp_dep_style3($value)
{
	if( $value == 'header_v3' ) return true;
	
	return false;
}
function vp_dep_style4($value)
{
	if( $value == 'four' ) return true;
	
	return false;
}
function vp_dep_style10($value)
{
	if( $value == 'predefined_color' ) return true;
	
	return false;
}
function vp_dep_style11($value)
{
	if( $value == 'custom_color' ) return true;
