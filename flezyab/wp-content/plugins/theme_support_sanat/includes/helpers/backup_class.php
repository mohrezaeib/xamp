...
class Bunch_Backup_class
{
	...
	{
		$this->_framework = '';//$GLOBALS['bunch_base'];
		
		
		$this->path = BUNCH_TH_ROOT.'/includes/resource/backup';
	...
	function revslider_export( $file = '' )
	{
		...
		foreach( $sliders as $k=>$s )
		{
			...
			foreach( $slides as $ss )
			{
				...
				if( $slider_id == bunch_set( $ss, 'slider_id' ) )
	...
	function theme_options_import($file = '')
	{
		...
		update_option( BUNCH_NAME.'_theme_options', $data );
		
		/** Update the front page */
		$front_page = get_page_by_title('Homepage One');
		$blog_page = get_page_by_title('Blog With Sidebar');
	...
	function newdir($path, $mode = '0777', $recursive = false)
	{
		...
		elseif( ! mkdir($path, $mode, $recursive)) wp_die( sprintf( __('System is not able to create backup directory, please create a directory named "backup" in "%s" manually and give it 0777 write permission.', BUNCH_NAME), $path));
	...
	function pseudo($options = array())
	{
		if( is_array( $options ) )
		{
			foreach($options as $k=>$v)
			{
				...
				{
					$options[$k] = str_replace(array(BUNCH_TH_URL, home_url(), get_option('admin_email')),array('{BUNCH_TH_URL}', '{HOME_URL}', '{ADMIN_EMAIL}'),$v);
				}
			}
		}else{
			...
			{
				$options = str_replace(array(BUNCH_TH_URL, home_url(), get_option('admin_email')),array('{BUNCH_TH_URL}', '{HOME_URL}', '{ADMIN_EMAIL}'),$options);
			}			
		}
	...
	function replace_pseudo($options = array())
	{
		if( is_array( $options ) )
		{
			foreach((array)$options as $k=>$v)
			{
				...
				{
					$options[$k] = str_replace(array('{BUNCH_TH_URL}', '{HOME_URL}', '{ADMIN_EMAIL}'), array(BUNCH_TH_URL, home_url(), get_option('admin_email')), $v);
				}
			}
		}else{
			$options = str_replace(array('{BUNCH_TH_URL}', '{HOME_URL}', '{ADMIN_EMAIL}'), array(BUNCH_TH_URL, home_url(), get_option('admin_email')), $options);
		}
	...
	function demo_content() 
	{
		...
		if ( ! class_exists( 'WP_Import' ) ) {
			$class_wp_importer = BUNCH_TH_ROOT."/includes/thirdparty/wordpress-importer/wordpress-importer.php";
			if ( file_exists( $class_wp_importer ) )
				require $class_wp_importer;
		}
