 ...
 * @version   1.0
 * @link      http://themeforest.net/user/yogsthemes
 * @author    Amir
...
class Bunch_Enqueue
{
	...
	function bunch_enqueue_scripts()
	{
		...
		$dark_color = ( sanat_set( $options, 'website_theme' ) == 'dark' ) ? true : false;
		
		$dark_color = ( sanat_set( $_GET, 'color_style' ) == 'dark' ) ? true : $dark_color;
		
		if(sanat_set($options, 'color_scheme') == 'color2') : $color = 'css/color2.css'; else: $color = 'css/color1.css'; endif;	
		...
		if( is_single() ) {
			...
			if( $format == 'gallery' ) wp_enqueue_script( array( 'sanat_jquery-flexslider' ) );
			if( $format == 'video' || $format == 'audio' ) wp_enqueue_script( array( 'sanat_jquery-fitvids' ) );
		}
		if( is_singular( 'bunch_projects' ) || $wp_query->is_posts_page || is_search() || is_tag() || is_category() || is_author() || is_archive() ) 
  		wp_enqueue_script( array('sanat_jquery-flexslider', 'sanat_owl.carousel', 'sanat_jquery-fitvids') );
		wp_enqueue_script( array('sanat_custom_script') );
		
		wp_add_inline_style( 'sanat-main-style', $this->wp_head() );
		$opt = _WSH()->option();
		$h_js = 'if( ajaxurl === undefined ) var ajaxurl = "'.esc_url( admin_url( 'admin-ajax.php' ) ).'";';
		wp_add_inline_script( 'sanat-main-script', $h_js );
		
		
	}
	
	function sanat_get_font_settings( $styles )
	{
		$opt = _WSH()->option();
		$protocol = ( is_ssl() ) ? 'https' : 'http';
		$font = array();
		...
		if( sanat_set( $opt, 'use_custom_font' ) ){
			...
			if( $h1 = sanat_set( $opt, 'h1_font_family' ) ) $font[$h1] = urlencode( $h1 ).':400,300,600,700,800';
			if( $h2 = sanat_set( $opt, 'h2_font_family' ) ) $font[$h2] = urlencode( $h2 ).':400,300,600,700,800';
			if( $h3 = sanat_set( $opt, 'h3_font_family' ) ) $font[$h3] = urlencode( $h3 ).':400,300,600,700,800';
		}
		
		if( sanat_set( $opt, 'body_custom_font' ) ){
			if( $body = sanat_set( $opt, 'body_font_family' ) ) $font[$body] = urlencode( $body ).':400,300,600,700,800';
		}
	...
	function wp_head()
	{
		...
		$style .= sanat_set( $opt, 'header_css' );
