<?php sanat_bunch_global_variable(); 
	...
	if( $wp_query->is_posts_page ) {
		...
		if(sanat_set($_GET, 'layout_style')) $layout = sanat_set($_GET, 'layout_style'); else
		$layout = sanat_set( $settings, 'archive_page_layout', 'right' );
		$sidebar = sanat_set( $settings, 'archive_page_sidebar', 'default-sidebar' );
		$bg = sanat_set($settings, 'archive_page_header_img');
		$title = sanat_set($settings, 'archive_page_header_title');
	}
	$layout = sanat_set( $_GET, 'layout' ) ? sanat_set( $_GET, 'layout' ) : $layout;
    ...
    <section class="page-title">
        <div class="auto-container">
            <h1><?php esc_html_e('Our Blog', 'sanat');?></h1>
