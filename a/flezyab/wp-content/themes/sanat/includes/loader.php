...
add_action( 'init', 'sanat_bunch_theme_init');


if( !function_exists( 'sanat_set' ) ) {
	function sanat_set( $var, $key, $def = '' )
	{
		if( !$var ) return false;
...
if( !function_exists('sanat_printr' ) ) {
	function sanat_printr($data)
	{
		echo '<pre>'; print_r($data);exit;
	}
}

if( !function_exists('sanat__font_awesome' ) ) {
	function sanat_font_awesome( $index )
	{
		$array = array_values($GLOBALS['_font_awesome']);
		if( $font = sanat_set($array, $index )) return $font;
		else return false;
	}
}
...
if( sanat_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...
function sanat_bunch_theme_init()
{
	 ...
	 */
	// metaboxes
	$tmpl_mb1  = include SANAT_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
	// * Create instances of Metaboxes
	foreach( $tmpl_mb1 as $tmb )  new VP_Metabox($tmb);
	
	$tmpl_mb1  = include SANAT_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
	...
	$tmpl_sg1  = SANAT_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
	$tmpl_sg2  = SANAT_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';
	...
	if( is_admin() ) {
		...
		if( sanat_set( $_GET, 'page' ) == 'sanat'.'_option' ) 
		{
			if( sanat_set( $_GET, 'bunch_dummydata_export' ) ){
	...
	if( function_exists( 'sanat_vc_map' )) 
	include_once get_template_directory() .  '/vc_map.php' ;
	
	// options
	$tmpl_opt  = SANAT_ROOT.'includes/vafpress/admin/option/option.php';
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'sanat'.'_theme_options',                      // options key in db, required
		'page_slug'             => 'sanat'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'sanat' ), // page title
		'menu_label'            => esc_html__( 'Theme Options', 'sanat' ), // menu label
	));
